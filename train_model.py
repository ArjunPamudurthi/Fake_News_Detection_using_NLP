...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Train Fake News Detector with TF-IDF (1-2 grams) + Logistic Regression"
    )
    ...
    #    Strip Reuters datelines like "CITY (Reuters) - " so the model
    #    learns content patterns, not just the Reuters header format.
    #    Also apply basic text cleaning for consistency with the app.
    dateline_re = re.compile(r"^[A-Z]+(?:\s[A-Z]+)*\s*\(Reuters\)\s*-\s*")
    ...
    for df in (df_real, df_fake):
        ...
        txt = txt.apply(lambda t: dateline_re.sub("", t) if isinstance(t, str) else t)
        combined = (title + " " + txt).str.strip()
        # Clean text: lowercase, remove URLs, non-alpha, extra spaces
        combined = combined.str.lower()
        combined = combined.apply(lambda t: url_re.sub("", t))
        combined = combined.apply(lambda t: non_alpha_re.sub(" ", t))
        combined = combined.apply(lambda t: multi_space_re.sub(" ", t).strip())
    ...
    # 5) Pipeline: Word TF-IDF + Char TF-IDF (captures writing style) + Logistic Regression
    pipe = Pipeline(
        steps=[
            (
                "features",
    ...
    #   Save the whole pipeline as 'pipeline.joblib' (vectorizer+model together)
    joblib.dump(pipe, outdir / "pipeline.joblib")
    #   Also keep separate parts for compatibility with existing CLI
    features = pipe.named_steps["features"]
    clf = pipe.named_steps["clf"]
    joblib.dump(features, outdir / "vectorizer.joblib")
